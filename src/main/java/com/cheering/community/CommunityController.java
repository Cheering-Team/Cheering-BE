...
public class CommunityController {
    ...
    public ResponseEntity<?> getCommunityInfo(@PathVariable("communityId") Long communityId, @AuthenticationPrincipal CustomUserDetails userDetails) {
        return ResponseEntity.ok().body(ApiUtils.success(HttpStatus.OK, "커뮤니티 정보 조회 완료", communityService.getCommunityInfo(communityId, userDetails.getUser())));
    }

    // 커뮤니티 가입
    @PostMapping("/communities/{communityId}/users")
    ...
    // (선수 커뮤니티 등록)
    @PostMapping("/teams/{teamId}/communities")
    public ResponseEntity<?> registerCommunity(@PathVariable("teamId") Long teamId, @RequestBody CommunityRequest.RegisterCommunityDTO requestDTO){
        communityService.registerCommunity(teamId, requestDTO);
    ...
    public ResponseEntity<?> createTeamCommunities(){
        communityService.createTeamCommunities();
        return ResponseEntity.ok().body(ApiUtils.success(HttpStatus.OK, "커뮤니티 등록 완료", null));
    }
}
