...
public interface MeetFanRepository extends JpaRepository<com.cheering.meetfan.MeetFan, Long> {
    ...
    @Query("SELECT mf FROM MeetFan mf JOIN mf.meet m JOIN m.match mt WHERE mf.fan.user = :user AND mf.role != :excludedRole")
    List<MeetFan> findByFanUserOrderByRoleExcludingLeft(@Param("user") User user, @Param("excludedRole") MeetFanRole excludedRole);
    ...
    @Query("SELECT mf FROM MeetFan mf WHERE mf.meet = :meet AND mf.fan.user = :user AND mf.role = :role")
    Optional<MeetFan> findByMeetAndUserAndRole(@Param("meet") Meet meet, @Param("user") User user, @Param("role") MeetFanRole role);
