...
public interface ReCommentRepository extends JpaRepository<ReComment, Long> {
    @Query("SELECT r FROM ReComment r WHERE r.comment = :comment AND r.isHide = false AND r.id NOT IN (SELECT rr.reComment.id FROM ReCommentReport rr WHERE rr.writer = :fan) AND r.writer NOT IN (SELECT b.to FROM Block b WHERE b.from = :fan) ORDER BY r.createdAt ASC")
